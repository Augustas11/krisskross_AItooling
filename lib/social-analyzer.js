    ...
    static async fetchInstagramMetrics(handle) {
        ...
        try {
            ...
            return {
                ...
                avgVideoViews: profile.latestPosts ?
                    Math.round(profile.latestPosts.reduce((acc, p) => acc + (p.videoViewCount || 0), 0) /
                        profile.latestPosts.filter(p => p.type === 'Video' || p.videoViewCount).length || 1) : 0,
                hasReels: profile.latestPosts && profile.latestPosts.some(p => p.type === 'Video' || p.videoViewCount > 0),
                latestPosts: profile.latestPosts // Pass full array for AI analysis
            };
