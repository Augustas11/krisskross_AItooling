...
export async function enrichAndTagLead(lead, options = {}) {
    ...
    if (perplexityResult?.data) {
        const pData = perplexityResult.data;

        // 1. Research Summary (Always present either as string or prop)
        if (typeof pData === 'string') {
            researchSummary = pData;
        } else {
            researchSummary = pData.summary || JSON.stringify(pData);

            // 2. Fill in gaps not found by basic scrape
            ...
            // 3. Fill in social gaps if Apify failed
            if (!metrics.followers && pData.instagram_followers) {
                lead.instagramFollowers = parseInt(pData.instagram_followers) || null;
            }
            if (!lead.engagementRate && pData.audience_vibe) {
                // If we have vibe but no rate, we can't fake rate, but we can store the vibe
                // Maybe put it in notes or tags later
            }
    ...
    lead.enrichmentHistory.unshift({
        ...
        method: 'Triple Threat (Apify + Perplexity + Claude)',
        status: 'Success',
        details: `Generated ${lead.tags?.length || 0} tags, Research: ${researchSummary ? 'Yes' : 'No'}, Metrics: ${metrics ? 'Yes' : 'No'}`
    });
