...
export async function enrichAndTagLead(lead, options = {}) {
    ...
    try {
        ...
        // Clean slate: Remove existing auto/ai tags to prevent accumulation of stale tags
        if (lead.tags) {
            lead.tags = lead.tags.filter(t => t.applied_by === 'manual');
        }

        // AI Comprehensive Analysis (if enabled and API key available)
        if (!options.skipAI && process.env.ANTHROPIC_API_KEY) {
            console.log(`[ENRICH] Analyzing lead context with AI...`);

            const latestPosts = metrics.latestPosts || [];

            // Pass the FULL lead context plus the newly fetched metrics
            const context = {
                ...lead,
            ...
            // Apply the AI generated tags
            if (aiTags && aiTags.length > 0) {
                aiTags.forEach(tag => {
                    // Remove old tags of same category if needed (optional, but cleaner)
