        ...
        try {
            ...
            if (pData) {
                ...
                if (typeof pData === 'string') {
                    ...
                    if (pData.contact_info && pData.contact_info.instagram_handle) {
                        const rawHandle = pData.contact_info.instagram_handle;
                        ...
                        if (rawHandle && rawHandle !== 'N/A' && !rawHandle.includes(' ') && rawHandle.length < 50) {
                            const foundHandle = SocialAnalyzer.extractInstagramHandle(rawHandle);
                            if (foundHandle) {
                                console.log(`[ENRICH] Perplexity found Instagram handle: ${foundHandle}`);
                                // Update lead if not present
                                if (!lead.instagram) {
                                    lead.instagram = foundHandle;
                                }
                            }
                        }
                    }
