...
export async function researchLead(lead) {
    ...
    // Hint from Site Scanner
    const instaHint = lead.scraped_instagram ? `We found a potential Instagram handle on their website: "@${lead.scraped_instagram}". VERIFY if this is their official account.` : '';

    // THE NEW "DEEP RESEARCH" PROMPT with domain disambiguation and contact priority
    const query = `
    ...
    ## PRIMARY OBJECTIVES (Execute First)
    1. Extract official contact information:
       - Website homepage URL
       - Instagram profile URL (search: "${name}" Instagram)
       - LinkedIn company page URL (search: "${name}" LinkedIn)
       - Contact email (search: site:${website || name} "contact" OR "email" OR "press" OR "support")
       - Phone number (search: site:${website || name} contact phone)
    
    IMPORTANT: Look for MULTIPLE emails if possible (e.g. support@, press@). Format as an array.
    
    IMPORTANT: If you find an Instagram handle in the search results text (e.g. "@outerknown"), constructs the full URL (https://instagram.com/outerknown).

    2. Format extracted data:
       \`\`\`json
       // EXAMPLE OUTPUT
       {
         "contact_info": {
            "website": "https://www.outerknown.com",
            "instagram": "https://instagram.com/outerknown",
            "linkedin": "https://www.linkedin.com/company/outerknown",
            "email": "customercare@outerknown.com",
            "phone": "+1-888-644-8044"
         },
         // ... other fields
       }
       \`\`\`

    ## SECONDARY OBJECTIVES (After contact extraction)
    ...
    2. "company_overview": Concise but specific summary of who they are. Include their domain/website to confirm identity.
    3. "target_audience": Detailed buyer persona (age, interests, values).
    4. "current_content_strategy": Analyze their likely Instagram/TikTok/Website presence. Do they use video? Is it static? Is it high quality?
    5. "pain_points": 3-5 SPECIFIC pain points inferred from their digital presence (e.g. "Instagram feed is 90% static images, missed opportunity for reach", "Website lacks founder story/trust triggers", "Inconsistent posting schedule").
    6. "strategic_recommendations": 3 specific ideas for how Short-Form Video (Reels/TikTok) could solve their pain points and drive sales.
