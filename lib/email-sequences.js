 ...
 */
export async function enrollLeadInSequence(leadId, sequenceId, startStep = 1, lastEmailSentAt = null) {
    ...
    try {
        ...
        // Create enrollment with last_email_sent_at if provided (for when enrolling after initial email)
        ...
        // If starting at step > 1, we should set last_email_sent_at to track when the prior email was sent
        if (lastEmailSentAt) {
            enrollmentData.last_email_sent_at = lastEmailSentAt;
        } else if (startStep > 1) {
            // If starting at step > 1 but no lastEmailSentAt provided, use current time
            // This ensures follow-up delays are calculated from now
            enrollmentData.last_email_sent_at = new Date().toISOString();
        }
